...
GravityTDS::GravityTDS():tdsSensorPin(32)
{
  sampleTimepoint = 0;
}
...
void GravityTDS::update()
{
   if(millis()-sampleTimepoint > 500U)  // every seconds, read the data from sensors and print them
   {
        ...
        voltage = (4095-analogValue)/(float)(ADCRANGE)*(float)(AREF);//convert ADC value to real voltage; AREF:5.0V
        // Serial.print(analogValue);
        // Serial.println("  ");
        ecvalue=(133.42*voltage*voltage*voltage - 255.86*voltage*voltage + 857.39*voltage)*1.0;//此处1.0原本为kValue，但是不知为何kValue的值为0，因此取默认值1.0
